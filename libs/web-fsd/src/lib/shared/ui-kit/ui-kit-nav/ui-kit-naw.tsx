...
const title = localStorage.getItem("current-text")
 ...
 const navMap: Record<NawPathEnum, { path: string; Icon : React.ElementType }> = {
    [NawPathEnum.DICTIONARY]:{ path: NawPathEnum.DICTIONARY, Icon : DescriptionIcon },
  [NawPathEnum.TESTS]:       { path:NawPathEnum.TESTS, Icon : SchoolIcon },
  [NawPathEnum.TRANSLATOR]:  { path: NawPathEnum.TRANSLATOR, Icon : TranslateIcon},

  [NawPathEnum.LIBRARY]:     { path: NawPathEnum.LIBRARY, Icon : MenuBookIcon },
   [NawPathEnum.CURRENT_TEXT]:{ path: `${NawPathEnum.CURRENT_TEXT}/${title}`, Icon : BookmarkIcon },

};

 const nawValues = [NawPathEnum.TESTS, NawPathEnum.TRANSLATOR,NawPathEnum.DICTIONARY,NawPathEnum.LIBRARY, NawPathEnum.CURRENT_TEXT]
