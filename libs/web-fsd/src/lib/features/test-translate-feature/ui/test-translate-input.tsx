...
export const TestTranslateFeature: React.FC<Props> = ({
  wordToTest,
  correctAnswer,
  onResult,
}) => {
  ...
  return (
    <article
      className={`w-full flex justify-center ${animateNext && 'animate-move-left'}`}
    >
      <div className=" pt-2 h-full flex flex-col items-end  ">
        ...
        <div className="flex flex-col flex-grow justify-center">
          <div className={` overflow-hidden flex flex-wrap w-full m-2 `}>
            <p
              className={` w-full  text-base mt-1 mb-1 ${shouldShake ? ' animate-move-right' : ''} `}
            >
              {wordToTest}
            </p>
