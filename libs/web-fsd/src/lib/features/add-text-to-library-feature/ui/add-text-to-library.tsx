...
const {title,content ,setTextTitle,handleFileUpload, handleSendText,isAlreadyExistText, fileAdded,setFileAdded, setIsAlreadyExist} = useAddTextToLibrary()
...
setIsAlreadyExist(null)
    ...
    return (
     ...
     <input className="hidden" id="file-input" type="file" accept=".txt,.md,.docx,.html" onChange={handleAddFile} />
...
<div className=" p-3 bg-red-900 absolute top-0 left-0 w-full h-full  flex flex-col items-end ">
 ...
 <InputUi textInfo={isAlreadyExistText}  value={title} onChange={(e)=>setTextTitle(e.target.value)}/>
 <ButtonIconUi Icon={FileUploadIcon} handleButton={()=>handleSendText({title,content}) } />
      </div> 
