  ...
  return (
    <div className="flex h-full flex-col  gap-2 flex-wrap   ">
      <div className="flex items-center h-fit w-full ">
        <InputUi
          placeholder={texts.library.inputPlaceholder}
          value={searchValue}
          ...
          return (
            <div key={t} className="  flex">
              <div
                ...
                onClick={() => handleClickByTitle({ isRenaming: !isReadOnly })}
              >
                <div className="flex-grow">
                  <InputUi
              ...
              {isReadOnly ? (
                <ButtonIconUi
                  ...
                  handleButton={() => handleRenameTrigger(t)}
                />
              ) : (
                <ButtonIconUi
                  Icon={DoneIcon}
                  handleButton={() => handleSendNewTitleButton(t, i)}
                />
              )}
              <RemoveTextFromLibraryButton
                index={i}
                onDelete={() => onDeleted(t)}
                rollback={() => rollback(t, i)}
                titleDelete={t}
              />
            </div>
          );
