...
export const useGetData = ({
    ...
    try {
      ...
      const wordsChunks = Array.from(
        ...
        (_, i) =>
          result.slice(
            i * testWordsLength,
            i * testWordsLength + testWordsLength,
          ),
      );
  ...
  }, [currentChunkIndex, setLoad, testWordsLength, setWords, setCurrentChunk]);

  const currentFalseWords = useMemo(() => {
    if (!currentWord) return { shuffled: [], progress: 0 };
    const slicedLength = 3;
    const currentWords = [
      ...currentChunk.filter((w) => w !== currentWord).slice(0, slicedLength),
      currentWord,
    ];
