...
export class QueueService {
  ...
  async addCreateNewUserTableJob(payload: NewUserRegPayload) {
    const job = await this.databaseQueue.add(
      JOB_NAMES.CRATE_NEW_USER_TABLE,
      payload,
    );
  ...
  async addAddWordToDictionaryJob(payload: AddWordJobPayload) {
    const job = await this.databaseQueue.add(
      JOB_NAMES.ADD_WORD_TO_DICTIONARY,
      payload,
    );
    return await job.finished();
  }
  ...
  async addRemoveFromDictionaryJob(payload: RemoveFromDictionaryobPayload) {
    const job = await this.databaseQueue.add(
      JOB_NAMES.REMOVE_FROM_DICTONARY,
      payload,
    );
    return job.finished();
  }

  async addUpdateDictionaryProgressJob(
